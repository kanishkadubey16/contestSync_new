...
const AdminPanel = () => {
  ...
  const fetchContests = async () => {
    try {
      const response = await api.get('/api/contests?limit=100');
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <p className="text-3xl font-bold text-purple-600">{overview.reminders || 0}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Participations</h3>
            <p className="text-3xl font-bold text-orange-600">{overview.participations || 0}</p>
          </div>
        </div>
      )}
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
              {leaderboard.map((user, index) => (
                <tr key={user.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{user.username}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{user._count.contestHistory || 0}</td>
